  ...
  <div class="content-wrapper">
    ...
    <div class="metric-grid">
      ...
      <div class="overview-card-layout">
        ...
        <div class="card-inner-box">{{ $requestsCount }}</div>
      </div>
      ...
      <div class="overview-card-layout">
        ...
        <div class="schedule-bg">
          <div class="card-inner-box-schedule metric-schedule-box mt-4">
            @if($nextScheduleDate)
            <span class="schedule-day-label">{{ $nextScheduleDate->format('l') }}</span>
            <p style="font-size: 1rem; margin-top: 15px;">
              {{ $nextScheduleDate->format('M') }}<br>
              {{ $nextScheduleDate->format('d') }}
            </p>
            @else
            <span class="schedule-day-label">N/A</span>
            @endif
          </div>
        </div>
      </div>
      ...
      <div class="overview-card-layout">
        ...
        <div class="card-inner-box">{{ $completedCount }}</div>
      </div>
    </div>
    ...
    <div class="card-metric-dark">
      ...
      @forelse($upcoming as $collection)
      ...
      @empty
      <p class="text-center text-white opacity-75">No upcoming collections</p>
      @endforelse
    </div>
