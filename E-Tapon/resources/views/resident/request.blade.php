      ...
      <div class="request-card-item">
        <div class="request-card-details">
          ...
          <p class="request-content">
            <strong>{{ $request['type'] }}</strong> • {{ $request['quantity'] }} kg
          </p>
          ...
          <!-- Status Badge -->
          <p class="request-content">
            <span class="status-badge status-{{ Str::slug($request['status']) }}">
              {{ $request['status'] }}
            </span>
          </p>

          <!-- Truck (if assigned) -->
          @if($request['truck'] !== 'Not assigned')
          <p class="request-content">Truck: {{ $request['truck'] }}</p>
          @endif

          <!-- CANCEL BUTTON (only if allowed) -->
          @if(in_array($request['status'], ['Pending', 'Assigned']))
          <form action="{{ route('resident.request.cancel', $request['request_id']) }}" method="POST" style="margin-top: 6px;" onsubmit="return confirm('Cancel this request?');">
            @csrf
            @method('PUT')
        ...
        <div class="request-checkmark-box">
          <svg xmlns="http://www.w3.org/2000/svg" width="150" height="150" fill="currentColor" class="bi bi-check-circle" viewBox="0 0 16 16">
            ...
            <path d="m10.97 4.97-.02.022-3.473 4.425-2.093-2.094a.75.75 0 1 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-1.071-1.05" />
          </svg>
        </div>
      </div>
